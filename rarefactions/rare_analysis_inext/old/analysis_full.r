...
# iNEXT helper functions
	...
	trocols <- c("darkorange", "forestgreen", "purple")
	...
	names(plotdata) <- habs
	for(i in 1:3){names(plotdata[[i]]) <- tros}
	...
	pdf("inext_plot_by_hab_and_tro.pdf")
	par(mfrow=c(2,2))
	for(i in 1:length(habs)){
		plot_troslist(plotdata[[i]], trocols, pchs=15:17, main=habs[i])
	}
	plot(0, type="n", xlim=c(0,1), ylim=c(0,4), axes=F, xaxt="n",
	 yaxt="n", xlab="", ylab="")
	points(x=c(0.1, 0.1, 0.1), y=1:3, pch=15:17, col=trocols, cex=2)
	segments(x0=0.0, x1=0.1, y0=1:3, y1=1:3, col=trocols)	
	segments(x0=0.1, x1=0.2, y0=1:3, y1=1:3, col=trocols, lty=2)	
	text(x=0.3, y=1:3, labels=tros, adj=c(0,0.5))

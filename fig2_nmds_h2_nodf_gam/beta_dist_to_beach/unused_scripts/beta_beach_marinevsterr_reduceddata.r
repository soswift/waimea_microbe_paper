...
# load stuff
	...
	# read in data
	otutable_fp <- "../rawdata/counts/mm_16s_hiseqs_abundance_table.csv"
	metadata_fp <- "../rawdata/counts/mm_16s_hiseqs_metadata_table.csv"
	taxonomy_fp <- "../rawdata/counts/mm_16s_hiseqs_taxonomy_table.csv"
...
# add combined tax column to taxonomy
	taxonomy$Taxonomy <- apply(X=taxonomy[,3:8], FUN=paste, MAR=1, collapse=";")

# convert otutable to matrix
	otutable <- as.matrix(otutable)
	rownames(otutable) <- otutable[,colnames(otutable)=="Group"]
	# remove first 1 columns
...
# get beta div mats by site
	...
	names(betas) <- sites
	save(list="betas", file="reduceddata_betas.rdata")
...
# plot mean and 95% CIs
	...
	pdf("reduceddata_beta_beach_allpairs_scatter.pdf")
	pcol <- "darkblue"
...
# make a plot
	...
	pdf("reduceddata_beta_beach_allpairs_violin.pdf")
	p
	dev.off()
